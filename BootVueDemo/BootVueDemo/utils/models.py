...
class User(db.Model):
    ...
    所以，如果你的模型类名为 User，
    那么 SQLAlchemy 默认会将其映射到名为 user 的数据库表。
    ...
    # 假如不是字典的情况下，因为我自己数据库查询到的结果是字典，所以我直接返回了，
    # 注意区分__dict__属性和to_dict()方法的区别.
    # 后者是我自己写的，一定程度上可以决定我需要暴露哪些字段给前端。
    # python 中的每个对象都有一个内置的 __dict__ 属性，它是一个字典，
    # 包含了该对象的所有属性和它们的值。
    # 你可以直接使用 __dict__ 属性来将对象转换为字典
    def to_dict(self):
        return {
            'id': self.id,
            'name': self.name,
